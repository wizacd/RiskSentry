...
export interface CompliancePassportData {
  vehicle: Vehicle;
  driver: Driver | null;
...
export function useCompliancePassport(unitCode: string) {
  const [data, setData] = useState<CompliancePassportData | null>(null);
  ...
  async function load() {
    ...
    const [{ data: driver }, { data: telemetry }, { data: p2h }, { data: notifs }] = await Promise.all([
      ...
      supabaseBrowser.from("notifications").select("*").eq("vehicle_id", vehicle.id).order("created_at", { ascending: false }),
    ]);

    const healthIndex = computeHealthIndex(p2h ?? [], telemetry ?? []);
  ...
  useEffect(() => {
    ...
    const channel = supabaseBrowser
      .channel(`compliance-passport-${unitCode}`)
